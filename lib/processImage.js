...
function prepareQueryString(queryString) {
  if (queryString.startsWith('jpegtran=')) {
    const result = ['jpegtran'];
    const remaining = queryString.slice(9).split(',');
      ...
      hijackResponse(
        ...
        (err, res) => {
          ...
          function makeFilterInfosAndTargetFormat() {
            const preparedQueryString = prepareQueryString(queryString);
          ...
          function startProcessing(optionalFirstChunk) {
            ...
            const pipeline = impro.createPipeline(
              { type, sourceMetadata, maxInputPixels: options.maxInputPixels },
