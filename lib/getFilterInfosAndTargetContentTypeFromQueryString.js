  ...
  function flushOperations() {
    if (operations.length > 0) {
      ...
      filterInfos.push({
        ...
        create: function() {
          var hasFormatChange = false;
          var enginesForOperations = this.operations.map(operation => {
            if (operation.name === 'setFormat') {
              hasFormatChange = true;
              if (sharpFormats.indexOf(operation.args[0]) === -1) {
                return 'gm';
              }
            }
          ...
          if (
            enginesForOperations.some(engineName => engineName === 'gm') ||
            (sourceContentType === 'image/gif' && !hasFormatChange)
          ) {
            engineName = 'gm';
          }

          if (engineName === 'gm' && sourceContentType === 'image/gif') {
