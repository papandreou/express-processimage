...
function isValidOperation(name, args) {
    ...
    switch (name) {
    case 'crop':
        return args.length === 1 && /^(?:east|west|center|north(?:|west|east)|south(?:|west|east)|attention|entropy)$/.test(args[0]);
    case 'rotate':
    ...
    case 'background':
        return args.length === 1 && /^#(?:[0-9a-f]{3}|[0-9a-f]{6}|[0-9a-f]{9}|[0-9a-f]{12}|[0-9a-f]{4}|[0-9a-f]{8}|[0-9a-f]{6})$/i.test(args[0]);
    case 'blur':
...
module.exports = function getFilterInfosAndTargetContentTypeFromQueryString(queryString, options) {
    ...
    var filterInfos = [];
    var defaultEngineName = options.defaultEngineName || (sharp && 'sharp') || 'gm';
            ...
            filterInfos.push({
                ...
                create: function () {
                    ...
                    if (engineName === 'gifsicle') {
                        ...
                        if (operations.some(function (operation) { return operation.name === 'resize'; })) {
                            sharpInstance.max();
                        }
                        ...
                        for (var i = 0 ; i < sharpOperationsForThisInstance.length ; i += 1) {
                            ...
                            if (operation.name === 'progressive' || operation.name === 'quality') {
                                var value = true;
                        ...
                        if (converterOptions) {
                            if (converterOperation) {
                                ...
                                sharpOperationsForThisInstance.push({name: this.targetContentType.replace(/^image\//, ''), args: [converterOptions]});
                            }
                        }
                        ...
                        readWriteStream.write = function (chunk) {
                            if (!spawned) {
                                ...
                                var seenData = false;
                                var hasEnded = false;
                                var gmInstance = gm(readStream, getMockFileNameForContentType(gmOperationsForThisInstance[0].sourceContentType));
                                if (options.maxInputPixels) {
