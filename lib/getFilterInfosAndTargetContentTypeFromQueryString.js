...
const createAnimatedGifDetector = require('animated-gif-detector');
...
let Gifsicle;
const isOperationByEngineNameAndName = { gm: {} };
const filterConstructorByOperationName = {};
...
Object.keys(gm.prototype).forEach(propertyName => {
  if (
...
const sharpFormats = ['png', 'jpeg', 'webp'];
if (sharp) {
  ...
  [
    ...
    'compressionLevel',
    'setFormat'
  ].forEach(sharpOperationName => {
    isOperationByEngineNameAndName.sharp[sharpOperationName] = true;
  });
}

const engineNamesByOperationName = {};

Object.keys(isOperationByEngineNameAndName).forEach(engineName => {
  Object.keys(isOperationByEngineNameAndName[engineName]).forEach(
    operationName => {
...
module.exports = function getFilterInfosAndTargetContentTypeFromQueryString(
  ...
  function checkSharpOrGmOperation(operation) {
    if (
      ...
      throw new errors.OutputDimensionsExceeded(
        `resize: Target dimensions of ${operation.args[0]}x${operation.args[1]} exceed maxOutputPixels (${options.maxOutputPixels})`
      );
    }
  }
  ...
  function flushOperations() {
    if (operations.length > 0) {
      let engineName = currentEngineName;
      const operationIndex = operationNames.length;
      operationNames.push('sharpOrGm');
      filterInfos.push({
        operationName: 'sharpOrGm',
        operations,
        usedQueryStringFragments: operations.map(
          operation => operation.usedQueryStringFragment
        ),
        create() {
          ...
          if (engineName === 'gifsicle') {
            ...
            operations.forEach(operation => {
              if (operation.name === 'resize') {
                ...
                gifsicleArgs.push(
                  '--crop',
                  `${operation.args[0]},${operation.args[1]}+${operation.args[2]}x${operation.args[3]}`
                );
              } else if (
                operation.name === 'rotate' &&
            ...
            const sharpOperationsForThisInstance = [].concat(operations);
            ...
            const resizeOptions = {
              fit: 'inside' // formerly known as .max()
            };
            const resizeOperation = operations.find(
              operation => operation.name === 'resize'
            );
            if (resizeOperation) {
              while (resizeOperation.args.length < 2) {
                resizeOperation.args.push(undefined);
              }
              resizeOperation.args.push(resizeOptions);
            }

            let converterOptions;
            let converterOperation;
            for (let i = 0; i < sharpOperationsForThisInstance.length; i += 1) {
              const operation = sharpOperationsForThisInstance[i];
              if (
                operation.name === 'progressive' ||
                ...
                ['withoutEnlargement', 'background'].includes(operation.name)
              ) {
                // sharp 0.22 removed these, map them to resize options
                resizeOptions[operation.name] = operation.args[0];
                sharpOperationsForThisInstance.splice(i, 1);
                i -= 1;
              } else if (operation.name === 'ignoreAspectRatio') {
                // sharp 0.22 removed this, map to the resize option
                ...
                resizeOptions.position = operation.args[0];
                sharpOperationsForThisInstance.splice(i, 1);
                i -= 1;
            ...
            sharpOperationsForThisInstance.forEach(operation => {
            ...
            const gmOperationsForThisInstance = [].concat(operations);
            ...
            readWriteStream.write = chunk => {
              if (!spawned) {
                ...
                for (
                  ...
                  const gmOperation = gmOperationsForThisInstance[i];
  ...
  keyValuePairs.forEach(keyValuePair => {
    ...
    if (matchKeyValuePair) {
      let operationName = decodeURIComponent(matchKeyValuePair[1]);
      ...
      if (
        ...
        if (operationName === 'resize') {
          if (typeof options.maxOutputPixels === 'number') {
            ...
            operationArgs = operationArgs.map(arg =>
              arg === '' ? undefined : arg
            );
          }
        }
        ...
        if (filters[operationName]) {
          ...
          filterInfos.push({
            ...
            create() {
              const sourceContentType = this.sourceContentType;
              ...
              duplexStream._read = size => {
                ...
                sharpInstance.metadata((err, metadata) => {
                  ...
                  if (typeof isAnimated === 'boolean') {
                    ...
                    animatedGifDetector.once('decided', isAnimated => {
                      metadata.animated = isAnimated;
                      proceed();
                    });
                  } else {
          ...
          const candidateEngineNames = engineNamesByOperationName[
            operationName
          ].filter(engineName => filters[engineName] !== false);
          if (candidateEngineNames.length > 0) {
            ...
            const sourceContentType = targetContentType;
            let targetFormat;
          ...
          const operationNameLowerCase = operationName.toLowerCase();
