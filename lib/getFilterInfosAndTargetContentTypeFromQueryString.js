...
Object.keys(isOperationByEngineNameAndName).forEach(engineName => {
  Object.keys(isOperationByEngineNameAndName[engineName]).forEach(operationName => {
  ...
  function flushOperations() {
    if (operations.length > 0) {
      ...
      filterInfos.push({
        ...
        operations,
        usedQueryStringFragments: operations.map(operation => operation.usedQueryStringFragment),
        create() {
          ...
          if (
            ...
            !this.operations.some(operation => operation.name === 'setFormat' &&
            ...
            if (
              ...
              Gifsicle &&
              this.operations.every(operation => operation.name === 'resize' ||
          ...
          if (engineName === 'gifsicle') {
            ...
            operations.forEach(operation => {
              if (operation.name === 'resize') {
                if (operation.args[0] === undefined) {
                  ...
                  if (
                    operations.some(operation => operation.name === 'ignoreAspectRatio')
                  ) {
                    gifsicleArgs.push(
                      '--resize',
            ...
            if (
              operations.some(operation => operation.name === 'resize')
            ) {
              sharpInstance.max();
            }
      ...
      if (
        ...
        if (operationName === 'resize') {
          if (typeof options.maxOutputPixels === 'number') {
            ...
            operationArgs = operationArgs.map(arg => arg === '' ? undefined : arg);
          }
        }
